...
// it is to set the background black
void clear_screen(){
        for (int x = 0; x < 320; x++)
                for (int y = 0; y < 240; y++)
                        plot_pixel (x, y, 0);
...
//drawing 4 points in square points of edge size 1 unit
...
// this is a fucntion that puts colour to screen
...
void draw_line(int x0, int y0, int x1, int y1, short int colour){
    ...
    int temp;
    if(is_steep){// making m less than eq 1
        swap(&x0, &y0);
        swap(&x1, &y1);
    }
    if(x0 > x1){// x1 poitns to the max x point and x0 min
        swap(&x0, &x1);
        swap(&y0, &y1);
    }
    ...
    int y = y0;
    int y_step;
    if(y0 < y1) y_step = 1;// means line upward facing
    else y_step = -1;
